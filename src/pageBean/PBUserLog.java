...
public class PBUserLog {
	...
	String strInfo="记录";
	String strUrl="UserLogManage";
	
	public PBUserLog() {
	}


	public PBUserLog(ArrayList arrayList, String pass_sort_id) {
	...
	public UserLog[] getNextPage() {
		...
		UserLog[] nsList = getUsList();
		this.description();// 打印相对页面信息
		return nsList;// 返回这个数组
	}

	public UserLog[] getPreviousPage() {
	...
	public UserLog[] getPostPage(int post) {
	...
	public UserLog[] getUsList() {
		...
		UserLog[] books = new UserLog[pageEndRow - pageStartRow + 1];// 定义一个数组为最后一条的指针减去最后一页开始的指针,得到要装多少条数据

		int j = 0;
		for (int i = pageStartRow; i < pageEndRow; i++) {// 循环得到数组中的指针对应的中间几条数据
			UserLog book = (UserLog) arrayList.get(i);// 生成得到的几个数据循环填充数据到UserLog中,这儿将生成相对应我多个UserLog对象
