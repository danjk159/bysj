...
public class PBCommentary {
	...
	String strInfo="文档";
	String strUrl="CommentaryManage";
	
	public PBCommentary() {
	}


	public PBCommentary(ArrayList arrayList, String pass_sort_id) {
	...
	public Commentary[] getNextPage() {
		...
		Commentary[] nsList = getUsList();
		this.description();// 打印相对页面信息
		return nsList;// 返回这个数组
	}

	public Commentary[] getPreviousPage() {
	...
	public Commentary[] getFristPage() {
	...
	public Commentary[] getPostPage(int post) {
	...
	public Commentary[] getUsList() {
		...
		Commentary[] books = new Commentary[pageEndRow - pageStartRow + 1];// 定义一个数组为最后一条的指针减去最后一页开始的指针,得到要装多少条数据

		int j = 0;
		for (int i = pageStartRow; i < pageEndRow; i++) {// 循环得到数组中的指针对应的中间几条数据
			Commentary book = (Commentary) arrayList.get(i);// 生成得到的几个数据循环填充数据到Commentary中,这儿将生成相对应我多个Commentary对象
	...
	public String getDescriptionsip(int searchName) {
